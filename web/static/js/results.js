(function () {
  ...
  function load(){
    ...
    fetch("/api/exam/result?exam_session_id="+encodeURIComponent(id)).then(function(r){return r.json().then(function(d){return {status:r.status,data:d};});}).then(function(res){if(res.status!==200){fail(res.status,res.data.message);return;}var r=res.data;document.getElementById("results-title").textContent=r.title||tr("exam.resultsTitle");document.getElementById("results-sub").textContent=(r.exam_kind||"")+" · "+(r.graded_at||"");root.innerHTML="";root.removeAttribute("aria-busy");var card=e("section","card");card.appendChild(e("h2","h3",tr("exam.summary")));card.appendChild(e("p",null,tr("exam.percentage")+": "+r.percentage));card.appendChild(e("p",null,tr("exam.points")+": "+r.earned_points+" / "+r.total_points));card.appendChild(e("p",null,tr("exam.answered")+": "+r.answered_count+" · "+tr("exam.blank")+": "+r.blank_count));card.appendChild(e("p",null,tr("exam.correct")+": "+r.correct_count+" · "+tr("exam.partial")+": "+r.partial_count+" · "+tr("exam.incorrect")+": "+r.incorrect_count));card.appendChild(e("p","hint","Provenance: "+(r.provenance&&r.provenance.scoring_policy_version||"no disponible")));root.appendChild(card);var nav=e("p");var review=e("a","button",tr("exam.viewReview"));review.href="review.html?xsid="+encodeURIComponent(id);nav.appendChild(review);root.appendChild(nav);}).catch(function(){fail(500,tr("exam.netError"));});
  }
