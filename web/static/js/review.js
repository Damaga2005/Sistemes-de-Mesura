(function () {
  ...
  function load(){
    ...
    fetch("/api/exam/review?exam_session_id="+encodeURIComponent(id)).then(function(r){return r.json().then(function(d){return {status:r.status,data:d};});}).then(function(res){if(res.status!==200){fail(res.status,res.data.message);return;}var d=res.data;document.getElementById("review-title").textContent=d.title||"Revisió";document.getElementById("review-sub").textContent=(d.exam_kind||"")+" · "+(d.graded_at||"");root.innerHTML="";root.removeAttribute("aria-busy");(d.questions||[]).forEach(function(q,i){var f=q.feedback||{},card=e("article","card");card.appendChild(e("h2","h3","Pregunta "+((q.position===undefined?i:q.position)+1)));card.appendChild(e("p",null,"Estat: "+(f.status||"—")+" · Punts: "+q.points_earned+" / "+q.points_available));if(f.student_answer!==null&&f.student_answer!==undefined)card.appendChild(e("p",null,"La teva resposta: "+f.student_answer));if(f.formula&&f.formula.html){var formula=e("div","formula");formula.setAttribute("aria-label","Fórmula");formula.innerHTML=f.formula.html;card.appendChild(formula);}(f.errors||[]).forEach(function(x){card.appendChild(e("p",null,"Error: "+(x.human||x.type||"—")+(x.severity_band?" · "+x.severity_band:"")));});(f.guidance||[]).forEach(function(x){card.appendChild(e("p","hint",x.hint||""));});root.appendChild(card);});var nav=e("p");var m=e("a","button button--secondary","Veure mastery");m.href="exam.html?xsid="+encodeURIComponent(id)+"#mastery";nav.appendChild(m);var p=e("a","button button--ghost","Practicar");p.href="practice.html";nav.appendChild(p);root.appendChild(nav);}).catch(function(){fail(500,"Error de xarxa.");});
  }
