...
| # | Decisión | Motivo | Alternativas descartadas |
|---|---|---|---|
| D179 | Test 2 de `tests/test_phase0.py` pasa de "los originales NO están en el repo" a "las fuentes registradas existen y su sha256 coincide con el manifest" | El contrato cambió con el merge `a790afa` (2026-09-09): `Tema 1/`…`Tema 10/` son ahora fuente canónica in-repo, inmutable y de solo lectura (AGENTS.md §Source of truth, README.md, `docs/ESTADO_DEL_PROYECTO.md` §6). El test viejo (`assert not ROOT.glob("Tema *")`) contradecía deliberadamente ese contrato y fallaba en árbol limpio. El nuevo test es **más estricto**: detecta borrado, ausencia y mutación byte a byte de cualquiera de las 91 fuentes | Borrar el test (perdería la garantía de integridad de fuente) / relajarlo a solo formato de hash (no detectaría corrupción) / dejar el FAIL (bloquea la suite y el release) |
| D180 | `app/student/mastery.py::_now()` usa `timespec="microseconds"` (antes `"seconds"`) | Defecto de determinismo reproducible (`test_21_mastery_deterministic` falla ~2/15 aislado y 1/1 bajo carga): el score es recency-weighted (`policy.update_score`, `w_i = 1 + 0.1*i`) y las señales se releen `ORDER BY created_at, event_id`. Con resolución de segundo, varios `submit()` del mismo segundo colisionan y el desempate por `event_id` (hash sha256) baraja el orden → score no determinista, violando "Determinismo total" (§49). Microsegundos preservan el orden de inserción para llamadas secuenciales | Tiebreak por `rowid` en el SELECT (obliga a tocar `replay()` y `test_22`, más superficie) / contador monótono en `MasteryEvent` (cambio de esquema) / marcar el test como flaky (oculta el defecto) |
| D181 | `.gitattributes`: `"Tema */*.html" text eol=crlf`, `"Tema */*.pdf" binary` | `data/source_manifest.json` hasheó el material del curso en su forma autorizada (CRLF). Con `core.autocrlf` variable por plataforma, un checkout en Linux (CI) entrega los HTML con LF y `test_2_source_integrity_matches_manifest` falla en las 81 fuentes HTML. Fijar el EOL de checkout hace el árbol reproducible en Windows y CI. Los blobs siguen en LF (sin `--renormalize` que reescriba nada); sólo cambia el smudge de checkout | Rehashar el manifest a LF (toca un artefacto congelado y el contrato de trazabilidad) / hacer el test EOL-agnóstico (deja de verificar bytes exactos) / no tener CI (bloquea la governance) |
| D182 | CI mínima (`.github/workflows/ci.yml`): push/PR a `main`, Python 3.12, `pip install -e ".[dev]"`, `pytest tests/ -q`, `python -m app.cli check` | Governance de release: CODE + TESTS + INTEGRITY en cada push/PR. Sin `GEMINI_API_KEY` → los `@NEEDS_KEY` se saltan y el tutor usa el extractivo determinista; sin secretos, sin billing | Matriz multi-Python (innecesaria para mono-máquina) / incluir Gemini real (coste + secreto en Actions) / seguir sin CI |

## F17 — Product UI / UX Redesign

| # | Decisión | Motivo | Alternativas descartadas |
|---|---|---|---|
| D183 | Tema visual dark-first con acento violeta (`--accent #6f4bff` fosc / `#6a45f0` clar); tema claro como override vía `prefers-color-scheme` (sin toggle). Identidad propia, no copia de la referencia | La referencia estética es un dashboard oscuro violeta, pero F17 §Fase 4 prohíbe "exceso de negro": `--bg #131118` es negro cálido violáceo, no `#000`. El sistema de tokens existente ya es semántico y dark-capable; solo cambian valores. Acento violeta es la mayor palanca de "admin tool" → "app educativa premium" | Mantener azul de sistema (conserva el aspecto genérico) / dark puro `#000` (prohibido) / toggle de tema (superficie de test extra, no pedido) / reescritura con framework (prohibida) |
| D184 | Inter self-hosted (`web/static/fonts/InterVariable.woff2`, SIL OFL 1.1) + 1 línea `".woff2": "font/woff2"` en `web/server.py` `_MIME` | Única excepción a "0 cambios de backend" en todo F17. La familia geométrica es parte del lenguaje visual premium; self-hosted mantiene el proyecto offline-first y sin dependencia de red (a diferencia de Google Fonts CDN) | Stack de sistema (aceptable pero menos premium) / Google Fonts CDN (rompe el ethos local-first, añade dependencia de red) / no cambiar `_MIME` (imposible servir el `.woff2`) |
| D185 | Etiquetas de estado de tema (`No iniciat` / `Reforçar` / `En progrés` / `Domini alt` / `Completat`) derivadas de `mastery.score` real con llindars congelados: `attempts==0\|\|null`→No iniciat; `<0.40`→Reforçar; `<0.75`→En progrés; `<0.95`→Domini alt; `>=0.95`→Completat (intervalos semiabiertos, solo si `attempts>0`) | Una UI de aprendizaje responde mejor a "¿qué debería hacer?" con una etiqueta que con un `37%` crudo. La regla es 100% derivada de números reales del backend, no inventa nada. Implementada una sola vez en `ui.js#statusFromMastery()` y reusada por Temari, Progrés y el dashboard. Llindars fijados aquí para que no puedan cambiar accidentalmente durante la implementación | Mostrar `%` crudo (no orienta la acción) / llindars sin documentar (deriva accidental) / pedir un `status_label` nuevo al backend para topics (cambio de contrato, prohibido) |
| D186 | Navegación reducida a funcionalidad real: `Inici · Temari · Pràctica · Tutor IA · Progrés · Exàmens`. Documents/Calendari salen de la nav principal; Historial se pliega dentro de Exàmens; `study.html`/`learning.html`/`history.html` quedan como stubs de redirección | F17 §Fase 5: eliminar de la nav lo no implementado. Documents/Calendari son stubs "properament". Historial es una proyección de sesiones de examen — pertenece junto a Exàmens, no como entrada separada. Los stubs `location.replace` preservan bookmarks | Mantener stubs en la nav (confunde implementado con no implementado) / borrar los HTML viejos (rompe bookmarks) / entrada de nav propia para Historial (llena la nav) |
| D187 | Perf budgets raised (total 120→220 KB, tokens 8→14 KB) + per-file ceilings added for shell.js/ui.js/i18n.js — richer design system + injected shell + i18n, still zero-dependency. The new numbers are ceilings, not space to fill: assets are minimized and no code is added solely to consume headroom. Record the final measured sizes so a future reviewer sees the margin. | Final measured sizes (F17 Task 14): tokens.css 6413 B (ceil 14 KiB) · app.js 4872 B (ceil 8 KiB) · shell.js 3599 B (ceil 10 KiB) · ui.js 10864 B (ceil 14 KiB) · i18n.js 14723 B (ceil 16 KiB, D-T13-b) · css-total 39495 B · perf-budget total 66598 B (ceil 225280). Every file is comfortably under its ceiling; nothing was padded or trimmed to hit a number. | Subir el límite sin registrar el margen (deriva silenciosa) / rellenar el presupuesto con código "por si acaso" (prohibido: ceilings, no targets) / bajar i18n.js a 12 KiB (el diccionario bilingüe completo de chrome no cabe, 14723 B medidos) |

## F19 — Windows Desktop App

| # | Decisión | Motivo | Alternativas descartadas |
|---|---|---|---|
| D188 | Escritorio Windows como wrapper `pywebview` + servidor Python embebido en un hilo daemon + empaquetado PyInstaller (`SistemesDeMesura.exe`), sin reescritura de la app. Único cambio de backend: `web/server.py` publica el puerto realmente enlazado en `$SM_PORT_FILE` (opt-in, después del `bind`, antes de `serve_forever()`, sin ventana TOCTOU) para que el launcher arranque con `--port 0`. | Una PWA no da ventana nativa y obliga a un service worker versionado; Electron/Tauri arrastran Node/Rust y rompen el contrato "0 dependencias de runtime / sin build". `app/paths.py` ya separa `package_dir()` (raíz de solo lectura → `_MEIPASS` al congelar) de `data_dir()` (→ `%LOCALAPPDATA%`), y `Bridge` ya copia la BD semilla en el primer arranque: la capa desktop es puramente aditiva. `pywebview`/`pyinstaller` son extras opcionales (`[desktop]`, `[build]`); la instalación base y el job Linux de CI no los tocan. | Mantener solo-web / `bind(:0)→close()→rebind` (carrera TOCTOU) / `pywebview` como dependencia obligatoria / bundlear el bootstrapper Evergreen de WebView2 (fuera de alcance de F19) / job Linux modificado para construir el EXE (se añade un job `windows-latest` independiente) |
| D189 | Cierre del Practice Learning Loop vía `AdaptiveLoop.step()` reutilizado en `PracticeWorkflow.submit_answer(adaptive=True)`; `next` opcional con pregunta/vista STEM, item y log; feedback pedagógico conservado con proyección anti-fuga (`calculations` sin `expected`, `units` sin `expected`, sin `correct_answer`) | El loop existía testeado pero sin llamador en runtime; duplicar `to_spec`+`generate` violaría regla 14 y el DTO amputaba `feedback`/`claims`/`provenance` ya generados | Nuevo AdaptiveEngine / FeedbackEngine propios / exponer Correction cruda (fuga de claves) / auto-avance sin botón (borra corrección visible) |
| D190 | Spacing persistente (`student_spacing`) + coverage KB en recomendaciones como códigos aditivos | Priority ya consumía errores vía mastery; faltaba memoria temporal `due` y noción de cobertura; pesos de Priority intactos y routing intacto (solo prefijos `raiz:` interpretados) | Recalibrar pesos Priority / segundo MasteryState / `target_error` en QuestionSpec (routing actual basta) / backfill histórico (reinterpreta datos) |
| D191 | LearningMode �nico (STUDY/PRACTICE/RECOVERY/EXAM) como filtros con fallback en AdaptiveLoop; EXAM rechazado expl�citamente | Cuatro motores paralelos (duplicaci�n) / tocar Priority (contrato certificado) / aislar mastery de examen (divergencia con grading F7) |
| D192 | Retention canonica derivada (Mastery+Spacing+History, sin tablas) + Recovery/Study con filtros por rango; FORGOTTEN exige historial fuerte + vencimiento + evidencia negativa (proxy de reinicio tras submit) | Tabla forgetting_memory / curva cientifica sin calibrar / tocar intervalos F10 o pesos Priority / segundo Adaptive Engine |
| D193 | CurriculumPlanner derivado (WHAT/WHEN) sobre Priority/Retention/Spacing/Errors sin segundo engine; buckets TODAY/WEEK/LATER + endpoint GET /api/learn/plan; sin migracion ni JS adaptativo | Recalibrar Priority / ontologia nueva / persistir planes / estimar tiempos sin datos / planificar EXAM |
